...
fn main() {
    ...
    for config in configs {
        if config.source.github.is_some() {
            for github in config.source.github.unwrap() {
                if github.user.is_none() {
                    continue;
                }
                ...
                for user in github.user.unwrap() {
                    info!("backuping repos for {}", user);
                    let repos = match github::get_user_repos(user, &token) {
                        Ok(r) => r,
                        Err(e) => panic!("{:?}", e),
                    };
                    ...
                    for repo in &repos {
                        ...
                        let mut remote = match repository.find_remote("origin") {
                            ...
                            Err(_) => {
                                ...
                                repository.remote("origin", &repo.clone_url).expect("problem adding remote")
                            }
                        };
                        ...
                        match remote.fetch(&["+refs/*:refs/*"], None, None) {
                            ...
                            Err(e) => {
                                error!(hoster = hoster, "a problem occured while fetching! {:?}", e)
                            }
                        }
                    }
                }
